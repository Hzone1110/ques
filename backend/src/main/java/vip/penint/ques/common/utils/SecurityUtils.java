...
public class SecurityUtils {
    ...
    private static final Integer ANSWER_FLAG_YES = 1;

    /**
     * 是否为答者
     */
    public static boolean isAnswer(SysUser user){
        return ANSWER_FLAG_YES.equals(user.getAnswerFlag());
    }
    ...
    public static boolean isComUser(){
        ...
        String comId = sysUser.getComId();
        if (StringUtils.isNotEmpty(comId) && !sysUser.getAdminFlag().equals(ADMIN_FLAG_YES)){
            return true;
        }else {
            return false;
        }
