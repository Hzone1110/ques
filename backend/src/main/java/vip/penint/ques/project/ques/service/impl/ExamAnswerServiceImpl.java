...
import vip.penint.ques.project.ques.mapper.IExamAnswerMapper;
...
public class ExamAnswerServiceImpl extends ServiceImpl<IExamAnswerMapper, ExamAnswer> implements IExamAnswerService {
    ...
    @Override
    public AjaxResult add(ExamAnswer examAnswer) {
        // 判断该场问卷一个人可以回答几次
        ...
        if (exam.getOpenType() == 2) {
            ...
            for (SysUser sysUser : selectUserList) {
                if (sysUser.getUserId().equals(SecurityUtils.getCurrUserId())) {
                    flag = true;
                }
            }
            if (!flag) {
        ...
        examAnswer.setUserId(Math.toIntExact(SecurityUtils.getCurrUserId()));
    ...
    public List<ExamAnswer> selectByUidAndExamId(String examId) {
        ...
        wrapper.eq(ExamAnswer::getUserId, SecurityUtils.getCurrUserId()).eq(ExamAnswer::getExamId, examId);
