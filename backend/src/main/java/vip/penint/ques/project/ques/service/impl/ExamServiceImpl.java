...
import vip.penint.ques.project.ques.domain.*;
import vip.penint.ques.project.ques.domain.vo.AnswerListVO;
import vip.penint.ques.project.ques.domain.vo.MyAnswerVO;
import vip.penint.ques.project.ques.mapper.IExamMapper;
import vip.penint.ques.project.ques.service.*;
...
public class ExamServiceImpl extends ServiceImpl<IExamMapper, Exam> implements IExamService {
    ...
    public void addExam(Exam exam) {
        exam.setComId(SecurityUtils.getCurrComId());
        ...
        if (StringUtils.isNotEmpty(exam.getId())) {
            if (this.getById(exam.getId()).getStatus().equals("1")) {
                throw new CustomException("该问卷已发布不支持修改");
            }
        }
    ...
    @Override
    public Exam selectExamInfo(String examId) {
        ...
        if (exam.getOpenType() == 2) {
            ...
            for (String s : arr) {
                SysUser user = userService.selectUserById(Long.valueOf(s));
                list.add(user);
            }
    ...
    @Override
    public void delExam(String examId) {
        if (this.getById(examId).getStatus().equals("1")) {
            throw new CustomException("该问卷已发布不支持删除");
        }
    ...
    public IPage<Exam> myList(Exam exam, QueryRequest queryRequest) {
        ...
        return baseMapper.myList(exam, SecurityUtils.getCurrUserId(), page);
    }
    ...
    public IPage<MyAnswerVO> myAnswer(Exam exam, QueryRequest queryRequest) {
        ...
        return baseMapper.myAnswer(exam, SecurityUtils.getCurrUserId(), page);
    }
    ...
    public IPage<AnswerListVO> examAnswerList(String examId, QueryRequest queryRequest) {
        ...
        return examAnswerService.examAnswerList(examId, page);
    }
