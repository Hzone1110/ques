...
import vip.penint.ques.project.ques.mapper.IQuMapper;
import vip.penint.ques.project.ques.service.IQuAnswerService;
...
public class QuServiceImpl extends ServiceImpl<IQuMapper, Qu> implements IQuService {
    ...
    public IPage<Qu> selectTQuList(Qu qu, QueryRequest queryRequest) {
        ...
        pages.getRecords().forEach(e-> {
            if (StringUtils.isNoneBlank(e.getComId())){
                e.setPublishName(companyService.selectSysCompanyById(e.getComId()).getCompanyName());
            }else {
                e.setPublishName("系统");
            }
        });
