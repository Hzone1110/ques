...
import vip.penint.ques.framework.web.domain.TreeSelect;
import vip.penint.ques.project.system.domain.SysMenu;
import vip.penint.ques.project.system.domain.SysUser;
import vip.penint.ques.project.system.domain.vo.MetaVo;
import vip.penint.ques.project.system.domain.vo.RouterVo;
import vip.penint.ques.project.system.mapper.SysMenuMapper;
import vip.penint.ques.project.system.mapper.SysRoleMenuMapper;
...
public class SysMenuServiceImpl implements ISysMenuService
{
    ...
    public List<SysMenu> selectMenuList(SysMenu menu, SysUser user) {
        ...
        menu.setTempId(sysCompany.getTempId());
    ...
    @Override
    public List<SysMenu> listMenusBySuperFlag(Integer superFlag) {
    ...
    @Override
    public Set<String> selectMenuPermsByAnswer() {
        List<String> perms = menuMapper.selectMenuPermsByAnswer();
        Set<String> permsSet = new HashSet<>();
        for (String perm : perms)
        {
            if (StringUtils.isNotEmpty(perm))
    ...
    public List<SysMenu> selectMenuTreeByUserId(SysUser user) {
        ...
        if (SecurityUtils.isAnswer(user)) {
            menus = menuMapper.listMenus4Answer();
            return getChildPerms(menus, 0);
        }
        ...
        if (SecurityUtils.isComAdmin(user)) {
            menus = menuMapper.listMenusByTempId(sysCompany.getTempId());
        } else {
            menus = menuMapper.selectMenuTreeByUserId(user.getUserId(), sysCompany.getTempId());
        }
        return getChildPerms(menus, 0);
    }
     ...
     */
    @Override
    public List<Integer> listCheckedMenusByTempId(String tempId)
    {
        return menuMapper.listCheckedMenusByTempId(tempId);
    }
    ...
    public int updateMenu(SysMenu menu)
    {
        ...
        System.out.println("是否超管：" + menu.getSuperFlag());
