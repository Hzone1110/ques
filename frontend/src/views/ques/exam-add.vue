  ...
  <div class="app-container">
    ...
    <el-card style="margin-top: 20px">
      ...
      >
        ...
        />
        <el-table-column label="发布租户" align="center" prop="publishName" />
      </el-table>
      <pagination
        v-show="total > 0"
        :total="total"
      ...
      <div v-show="postForm.openType === 2">
        ...
        >
          ...
          <template slot-scope="scope">
            <el-tag v-if="scope.row.attributes.indexOf(1) != -1">{{options[0].label}}</el-tag>
            <el-tag v-if="scope.row.attributes.indexOf(2) != -1">{{options[1].label}}</el-tag>
            <el-tag v-if="scope.row.attributes.indexOf(3) != -1">{{options[2].label}}</el-tag>
            <el-tag v-if="scope.row.attributes.indexOf(4) != -1">{{options[3].label}}</el-tag>
          </template>
          </el-table-column>
          <el-table-column
            label="姓名"
...
export default {
  ...
  methods: {
    ...
    async getUserList() {
      ...
      let response = await listUser(this.userQueryParams);
